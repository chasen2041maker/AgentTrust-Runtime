...
- 危险 shell `argv` 规则、默认拒绝的兼容 shell 工具，以及真实 `argv` 基准用例。
- 以已验证 trace 回放恢复/审批/事实，恢复前重新校验策略快照摘要。
- evidence 绑定的备份恢复、MCP 显式模拟、验证后 OTel 导出、审批过期、会话串行化与自定义工具回注册。

### v0.5.2 审批、恢复与并发收敛

- 策略 `allow` 可以覆盖已注册工具的普通默认 `ask`，同时保留未注册工具和注册表硬拒绝的不可提升边界。
- 审批参数提供脱敏预览、摘要绑定和默认一小时 TTL；运行时模式与审批模式解耦，框架会话默认 deferred。
