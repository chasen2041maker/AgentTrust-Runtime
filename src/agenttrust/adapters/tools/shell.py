...
def shell(intent: ToolIntent, project_root: Path) -> ToolResult:
    ...
    if not isinstance(argv, list) or not argv or not all(isinstance(item, str) and item for item in argv):
        return ToolResult(
            ...
            tool_name=intent.tool_name,
            status="error",
            error="shell requires a non-empty argv list of strings",
        )
...
def unsafe_shell_command(intent: ToolIntent, project_root: Path) -> ToolResult:
    """Run legacy command strings only through an explicitly named unsafe tool."""
    ...
    if not isinstance(command, str) or not command:
        return ToolResult(
            ...
            tool_name=intent.tool_name,
            status="error",
            error="unsafe_shell_command requires a non-empty string command",
        )
    started = time.perf_counter()
    completed = subprocess.run(
        command,
    ...
    return ToolResult(
        ...
        output_preview=output[:500],
        output_digest=_digest_text(output),
        metadata={"exit_code": completed.returncode, "duration_ms": duration_ms, "unsafe_compatibility": True},
        error=None if status == "ok" else f"unsafe shell exited with {completed.returncode}",
    )
